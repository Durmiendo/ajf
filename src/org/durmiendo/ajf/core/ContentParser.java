...
public class ContentParser extends mindustry.mod.ContentParser {
    ...
    ObjectMap<Class<?>, FieldParser> classParsers = new ObjectMap<>(){{
        ...
        put(BulletType.class, (type, data) -> {
            ...
            data.remove("type");
            BulletType result = (BulletType)make(bc, data);
            readFields(result, data);
            return result;
        });
        ...
        put(DrawBlock.class, (type, data) -> {
            if(data.isString()){
                //try to instantiate
                return make(resolve(data.asString()), data);
            }
        ...
        put(Objectives.Objective.class, (type, data) -> {
            ...
            data.remove("type");
            Objectives.Objective obj = make(oc, data);
            readFields(obj, data);
            return obj;
        });
    ...
    private ObjectMap<ContentType, TypeParser<?>> parsers = ObjectMap.of(
            ContentType.block, (TypeParser<Block>)(mod, name, value) -> {
                ...
                if(locate(ContentType.block, name) != null){
                    if(value.has("type")){
                        ...
                        block = make(resolve(value.getString("type", ""), Block.class), mod + "-" + name, value);
                    }else{
            ...
            ContentType.unit, (TypeParser<UnitType>)(mod, name, value) -> {
                ...
                if(locate(ContentType.unit, name) == null){
                    ...
                    unit = make(resolve(value.getString("template", ""), UnitType.class), mod + "-" + name, value);
            ...
            ContentType.weather, (TypeParser<Weather>)(mod, name, value) -> {
                ...
                if(locate(ContentType.weather, name) != null){
                    ...
                    readBundle(ContentType.weather, name, value);
                    item = make(resolve(getType(value), ParticleWeather.class), mod + "-" + name, value);
                    value.remove("type");
                }
                currentContent = item;
            ...
            ContentType.liquid, (TypeParser<Liquid>)(mod, name, value) -> {
                ...
                if(locate(ContentType.liquid, name) != null){
                    ...
                    readBundle(ContentType.liquid, name, value);
                    liquid = make(resolve(value.getString("type", null), Liquid.class), mod + "-" + name, value);
                    value.remove("type");
                }
    ...
    <T> T make(Class<T> type, JsonValue value){
        try{
            String builder = """
                    (function() {
                        return function() {
            ...
            for (JsonValue entry = value.child; entry != null; entry = entry.next)
                if (entry.name.startsWith("function-")) {
                    String names = entry.name.substring("function-".length());
                    overrides.append(names).append("{").append(entry.asString()).append("},");
                    none = false;
                }
            ...
            String ss = Strings.format(builder, AJF.m.get(type), overrides);
            Scripts s = Vars.mods.getScripts();



            Function f = (Function) s.context.evaluateString(s.scope, ss, "", 1);
            return (T) f.call(s.context, s.scope, s.scope, new Object[]{});
        }catch(Exception e){
            throw new RuntimeException(e);
        }
    }

    private <T> T make(Class<T> type, String name, JsonValue value){
        try{
            ...
            String ss = Strings.format(builder, AJF.m.get(type), overrides);
            Scripts s = Vars.mods.getScripts();
            Function f = (Function) s.context.evaluateString(s.scope, ss, "", 0);
            return (T) f.call(s.context, s.scope, s.scope, new Object[]{name});
        }catch(Exception e){
            throw new RuntimeException(e);
        }
    }
